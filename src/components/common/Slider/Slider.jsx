...
import { MotionText } from "../SVG/MotionText"
import styles from "./Slider.module.css"

/**
 * Generic SVG range slider, decoupled from any context.
 *
 * Visual state (localMin/localMax), hover state and dragging state are all
 * managed internally, but each can be controlled from the outside by passing
 * the corresponding prop pair — useful when a sibling component (e.g. a
 * distribution chart) needs to stay in sync with the slider.
 */
export function Slider({
  ...
  hasRange = false,
  isInverted = false,
  colors = { min: "white", max: "white" },
  hasHoverGuides = true,
  // Optional controlled visual state (falls back to internal state)
  localMin: localMinProp,
  setLocalMin: setLocalMinProp,
  localMax: localMaxProp,
  setLocalMax: setLocalMaxProp,
  // Optional controlled hover / dragging state
  hoveredSvg: hoveredSvgProp,
  setHoveredSvg: setHoveredSvgProp,
  isDragging: isDraggingProp,
  onDraggingChange,
  // Notified with the cursor x position (svg coordinates) on every mouse move
  onCursorMove,
  // Scale mapping values to pixels; defaults to a linear scale over the width
  xScale: xScaleProp,
  // Custom track renderer: ({ minCursorPosition, maxCursorPosition, sliderWidth, height, cursorWidth }) => node
  renderTrack,
}) {
  ...
  const [internalLocalMin, setInternalLocalMin] = useState(selectionMin)
  const [internalLocalMax, setInternalLocalMax] = useState(selectionMax)
  const localMin = localMinProp ?? internalLocalMin
  const localMax = localMaxProp ?? internalLocalMax
  const setLocalMin = setLocalMinProp ?? setInternalLocalMin
  const setLocalMax = setLocalMaxProp ?? setInternalLocalMax

  const [internalHovered, setInternalHovered] = useState(false)
  const hoveredSvg = hoveredSvgProp ?? internalHovered
  const setHoveredSvg = setHoveredSvgProp ?? setInternalHovered

  const [internalDragging, setInternalDragging] = useState(false)
  const isDragging = isDraggingProp ?? internalDragging
  ...
  const xScale = useMemo(
    () => xScaleProp ?? scaleLinear([min, max], [0, width]),
    [xScaleProp, min, max, width],
  )
  ...
  useEffect(() => {
    onCursorMove?.(svgCursorPosition.x)
  }, [svgCursorPosition])
  ...
  return (
    <div className={styles.sliderContainer}>
      <svg
        ...
        className={styles.svgSlider}
        onMouseDown={handleTrackClick}
        onMouseEnter={() => setHoveredSvg(true)}
        onMouseLeave={() => setHoveredSvg(false)}
      >
        {renderTrack ? (
          renderTrack({ minCursorPosition, maxCursorPosition, sliderWidth, height, cursorWidth })
        ) : (
        ...
        {hasHoverGuides && hoveredSvg && !isDragging && closerCursor !== "over" && (
          <g>
            <motion.g
