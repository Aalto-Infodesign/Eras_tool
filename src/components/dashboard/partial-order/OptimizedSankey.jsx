...
// --- Sub-component for rendering the interactive silhouettes over a link ---
/*
function SilhouetteGroup({
  link,
  palette,
  hoveredSilhouette,
  setHoveredSilhouette,
  ...
  const segmentsBySilhouette = Object.entries(groupBy(link.segments, "silhouette")).map(
    ([id, value]) => ({ id, value })
  )

  const silhouetteExtent = extent(segmentsBySilhouette, (d) => d.value.length)
  const silhouetteScale = scaleLinear().domain(silhouetteExtent).range([4, 40])

  const linkGenerator = sankeyLinkHorizontal()
  const [index_S] = link.source.id.split("_")

  const totalSilhouetteHeight = segmentsBySilhouette.reduce((sum, s) => sum + s.value.length, 0)
  const totalGapSpace = link.width - totalSilhouetteHeight
  const segmentGap = 2
  //   const segmentGap = Math.max(0, totalGapSpace / (segmentsBySilhouette.length + 1))

  let accumulatedHeight = 0

  return segmentsBySilhouette.map((segment, i) => {
    const segmentHeight = silhouetteScale(segment.value.length)
    const segmentTopOffset = segmentGap + accumulatedHeight
    accumulatedHeight += segmentHeight + segmentGap

    const syntheticLink = {
      ...link,
      width: segmentHeight,
      y0: link.y0 - link.width / 2 + segmentTopOffset + segmentHeight / 2,
      y1: link.y1 - link.width / 2 + segmentTopOffset + segmentHeight / 2,
    }
    ...
    const isVisible = isSameHoveredLink || isHovered
        ...
        {isVisible && (
          <motion.path
            ...
            animate={{
              ...
              strokeWidth: isHovered ? SILHOUETTE_HOVER_STROKE_WIDTH : segmentHeight,
            }}
            exit={{ pathLength: 0 }}
            transition={{
            ...
            onMouseEnter={() => {
              setHoveredSilhouette({ id: segment.id, link: link, size: segment.value.length })
              //console.log(link)
            }}
