...
import { useDerivedData } from "../../../contexts/DerivedDataContext.jsx"

export function ExportIDs() {
  const { selectedIDs } = useDerivedData()

  return (
    <motion.div layout className="export-ids-panel">
      {selectedIDs.length === 0 && (
        <motion.p style={{ minHeight: 150, marginBottom: 10 }} layout>
          No silhouette selected
        </motion.p>
      )}
      {selectedIDs.length > 0 && (
        <Virtuoso
