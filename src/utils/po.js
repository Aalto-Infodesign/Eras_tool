...
export const po = {
  ...
  //       const context = us
  //         .concat(ds)
  //         .concat(Array.from({ length: poset.elements.length }))
  //         .map((e) => poset.elements.indexOf(e)) //! isn't there one less???

  //       return Array.from({ length: poset.dominanceMatrix.length }, (_, i) =>
  //         context.includes(i) ? 1 : 0
  //       )
  //     })
  //     return {
  //       vectors: vectors,
  ...
  //       return list
  //     }
  //     const upVals = getNext(
  //       -1,
  //       0,
  //       Array.from({ length: poset.elements.length }, () => 0)
  //     )
  //     const downVals = getNext(
  //       1,
  //       0,
  //       Array.from({ length: poset.elements.length }, () => 0)
  //     ) //?? []
  ...
  //       return Array.from({ length: poset.elements.length }, (_, i) =>
  //         context.includes(i) ? values[i] : 0
  //       )
  //     })
  ...
  circularEmbedding: function (
    profiles,
    ids = Array.from({ length: profiles.length }, (_, n) => n),
    cells = 12,
    iterations = 100,
    learningRate = 0.1,
    seed = 42
    ...
    const transposed = Array.from({ length: profiles[0].length })
      .fill(0)
      .map((_, tCol) => profiles.map((row) => row[tCol]))
    ...
    const subspaceExtent = (subspace) => {
      const transposed = Array.from({ length: subspace[0].length })
        .fill(0)
        .map((_, tCol) => subspace.map((row) => row[tCol]))
      const extent = (l) => [Math.min(...l), Math.max(...l)]
    ...
    const getAdjacetns = (list, i) => {
      ...
      return [
        { neuron: list[prev], position: prev },
        { neuron: list[next], position: next },
      ]
    }
    ...
    return {
      ...
      getNeuron: function (id) {
        return this.neurons.find((neuron) => neuron.bmusID.includes(id)).position
      },
      toHue: d3.scaleLinear([0, cells], [0, 360]),
    }
  },
  createPoset: (input, elementNames = null) => {
    let isDominanceMatrix =
      Array.isArray(input[0]) && typeof input[0][0] === "number" && input[0].length === input.length
    ...
    if (isDominanceMatrix) {
      ...
      elements =
        elementNames || Array.from({ length: dominanceMatrix.length }, (_, i) => `profile_${i}`)
      ...
      dominanceMatrix = Array.from({ length: elements.length }, () =>
        Array.from({ length: elements.length }).fill(0)
      )
    ...
    const poset = {
      ...
      getUpset: function (element, mask = false) {
        ...
        return mask
          ? this.elements.map((_, i) => (this.dominanceMatrix[index][i] === 1 ? 1 : 0))
      ...
      getDownset: function (element, mask = false) {
        const index = this.elements.indexOf(element)
        return mask
          ? this.elements.map((_, i) => (this.dominanceMatrix[i][index] === 1 ? 1 : 0))
      ...
      getCovMatrix: function () {
        ...
        for (let i = 0; i < n; i++) {
          for (let j = 0; j < n; j++) {
            ...
            if (i !== j && coveringMatrix[i][j] === 1) {
              // We keep i !== j as self-loops typically don't represent dominance
      ...
      getCovering: function (element) {
        ...
        return this.getCovMatrix()
          [row].map((e, n) => (e === 1 ? n : -1))
          .filter((e) => e !== -1)
          .map((n) => this.elements[n])
      },
      ...
      enrich: function () {
        ...
        this.feature = function (key, value) {
          if (value === undefined) {
            ...
            Object.keys(this.features).forEach(
              (node, row) =>
                (this.features[node][key] = JSON.parse(
                  JSON.stringify(value(node, this.features[node]))
                ))
            )
          } else {
            Object.keys(this.features).forEach(
              (node) => (this.features[node][key] = JSON.parse(JSON.stringify(value)))
            )
          }
          return this
        }
        ...
        this.eachFeature = function (feature, f = (node, feat) => [node, feat]) {
          Object.keys(this.features).forEach((node) => f(node, this.features[node][feature]))
          return this
        }
    ...
    function drawHasse(container, poset = this) {
      ...
      poset
        ...
        .feature(
          "depth",
          (d) => poset.layers.length - poset.layers.findIndex((layer) => layer.includes(d))
        )
      ...
      poset.feature(
        "depth",
        (node) =>
          poset.features[node].depth ||
          sBScale(poset.features[node].subScore) +
            (sPScale(poset.features[node].supScore) - sBScale(poset.features[node].subScore)) / 20
      )
      ...
      const levels = poset.elements.map(
        (el) => poset.layers.length - poset.layers.findIndex((l) => l.includes(el))
      ) //topologicalSort(edges, numNodes);
      ...
      const nodePositions = levels.map((level, i) => ({
        ...
        y: height - ((height / (Math.max(...levels) + 1)) * level + 50), // Maxima at top, infima at bottom
        depth:
          Object.values(poset.features)[i].depth * 60 ||
          height - ((height / (Math.max(...levels) + 1)) * level + 50),
      }))
      ...
      const nodeSelection = svg
        ...
        .attr("fill", (d) =>
          poset.features[poset.elements[0]]?.fill
            ? poset.features[poset.elements[d.id]].fill
            : "lightgray"
        )
        ...
        .call(
          d3
            ...
            .on("drag", function (event, d) {
              d.x = event.x
              d.y = event.y
              d3.select(this).attr("cx", d.x).attr("cy", d.depth)
              updateEdges()
            })
            .on("end", function () {
      ...
      function updateEdges() {
        ...
        labelSelection.attr("x", (d) => d.x).attr("y", (d) => d.depth)
      }
    ...
    function circularSOM(
      ...
      const neurons = Array.from({ length: cells }, (_, n) => ({
        position: n,
        weights: [...Array(profiles[0].length).fill(0)].map(
          (_, n) => extentProfiles[n][0] + random() * (extentProfiles[n][1] - extentProfiles[n][0])
        ),
        bmus: [],
        bmusID: [],
        ref: [],
    ...
    function hSep(adjacencyMatrix) {
      ...
      const shortestPaths = Array.from({ length: n }, (_, i) =>
        Array.from({ length: n }, (_, j) => (i === j ? 0 : adjacencyMatrix[i][j] > 0 ? 1 : INF))
      )
          ...
          if (adjacencyMatrix[u][v] > 0) {
            for (let k = 0; k < n; k++) {
              if (longestPaths[k][u] > -INF) {
                longestPaths[k][v] = Math.max(longestPaths[k][v], longestPaths[k][u] + 1)
              }
            }
          }
        }
      ...
      const scores = ratios.map((row) =>
        row.map((ratio) => (isNaN(ratio) ? 1 : ratio === null ? 0 : ratio))
      )
    ...
    function setLayers(depth, impute) {
      ...
      if (depth === undefined) {
        ...
        this.eachFeature(depth, (name, feature) =>
          layers[feature] === undefined ? (layers[feature] = [name]) : layers[feature].push(name)
        )
        this.layers = layers
      } else if (typeof depth === "function") {
        const layers = []
        this.elements.forEach((name) =>
          layers[depth(name)] === undefined
            ? (layers[depth(name)] = [name])
            : layers[depth(name)].push(name)
        )
        this.layers = layers
      }
    ...
    poset.setLayers = setLayers //? consider adding substructures
    ...
    function coloringLogic(layer, iNeg, i, delta, seed = 12) {
      if (iNeg === 0) {
        ...
        const categories = po.circularEmbedding(
          vectors,
          ids,
          Math.ceil(ids.length * (ids.length / 2)),
          1000,
          0.1,
          seed
        )
        //po.visualizeEmbedding(categories)
        const rootsCategorized = categories.neurons
          .filter((neuron) => neuron.bmus.length > 0)
          .map((neuron) =>
            neuron.ref.map((ref) => {
        ...
        polarRepulsion(rootsCategorized, delta, 1).forEach(
          (pNode) => (
            (poset.features[pNode.id]["pX"] = pNode.x),
            (poset.features[pNode.id]["pY"] = pNode.y),
            (poset.features[pNode.id]["pTheta"] = pNode.theta),
            (poset.features[pNode.id]["pAlpha"] = Math.sqrt(
              Math.pow(pNode.x, 2) + Math.pow(pNode.y, 2)
            ))
          )
        )
      } else {
        const parents = poset.layers
          .slice(i + 1)
          .flat()
          .map((node) => ({
            id: node,
        ...
        const points = layer.map((node) => {
          ...
          const alpha =
            ancestors.map((a) => poset.features[a.id].pAlpha).reduce((a, b) => a + b) /
            ancestors.length
        ...
        polarRepulsion(inputPoints, delta, inputPoints[0].alpha).forEach((pNode, n) => {
          ...
          if (bp) {
            ...
            ancestorsPerPoint[pNode.id].forEach((a) => {
              poset.features[a].pTheta =
                pNode.theta + (poset.features[a].pTheta - pNode.theta) / 100
            })
          }
        })
      }
    ...
    function color(delta = 1, lThreshold = 40, hThreshold = 80, flip = false, seed = 12) {
      ...
      poset.feature("fill", (node) => {
        ...
        let l = flip
          ? 100 - (lThreshold + d.depth * degree + remainder)
