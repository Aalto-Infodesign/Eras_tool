  ...
  createPoset: (input, elementNames = null) => {
    ...
    const poset = {
      ...
      exportStructure: function () {
        //const structure = Object.fromEntries(Object.entries(poset).filter(e=>typeof e[1] !== 'function'))
        this.type = "poset"
    ...
    function setSubstructure(name, depth) {
      ...
      if (typeof depth === "string") {
        ...
        this.eachFeature(depth, (name, feature) =>
          substructure[feature] === undefined
            ? (substructure[feature] = [name])
            : substructure[feature].push(name),
        )
        this.analytics.substructures[name] = substructure
      } else if (typeof depth === "function") {
        const substructure = []
        this.elements.forEach((name) =>
          substructure[depth(name)] === undefined
            ? (substructure[depth(name)] = [name])
            : substructure[depth(name)].push(name),
        )
        this.analytics.substructures[name] = substructure
      }
